...
def nibio_data_transform(daily_2014_2020,target):
	...
	data = daily_2014_2020.values
	scaler = MinMaxScaler(feature_range=(0, 1))
	scaler1 = MinMaxScaler(feature_range=(0, 1))
	scaler = scaler.fit(daily_2014_2020)
	scaler1 = scaler1.fit(ds)
	# convert all variables to array.

	return data,scaler1,scaler

def generatedatanibio(datadir,dataname):
    year_start, year_end = 2014, 2020
    daily = []
    for year in range(year_start,year_end+1):
        daily_2014_2020_new = pd.read_csv(datadir+dataname.format(year)+".csv",
                                          delimiter = ",",
                                          header = 0,
                                          names = ["Time", "TM","RR","TJM10","TJM20"]).to_numpy().tolist()
        for a in daily_2014_2020_new:
            daily.append(a)
    daily_2014_2020 = pd.DataFrame(daily, columns = ["Time", "TM","RR","TJM10","TJM20"])
    data, scaler1, scaler = nibio_data_transform(daily_2014_2020,"TJM20")
...
def train_lstm(model,lr,total_epoch,train_loader,data_valid_x,data_valid_y,model_save_fn):
    if (os.path.exists(model_save_fn)):
        ...
        for epoch in range(total_epoch):
            ...
            # ########################################################
            print(epoch,'train_Loss:', train_loss.item(),'validation_Loss:', valid_loss.item())
...
if __name__ == '__main__':
    ...
    p.add_argument('--dataname', type=str, default='weather_data_hour_stID5_y{}', help='Location to data')
    ...
    p.add_argument('--source', type=str, default='nibio', help='name for data source')
    args = p.parse_args()
