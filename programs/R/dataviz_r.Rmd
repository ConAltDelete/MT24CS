...
if(!file.exists(csv_na_file_name)){
  file.create(csv_na_file_name)
}
cat(csv_title_cumsum_count,
            file = csv_na_file_name,sep="\n")
if(!file.exists(csv_dekkning_file_name)){
  file.create(csv_dekkning_file_name)
}
cat(csv_title_dekkning,
            file = csv_dekkning_file_name,sep="\n")
if(!file.exists(report_name)){
  file.create(report_name)
}
...
for(id in all.id){
    # beginning plot
    pdf(file = paste0(PLOTS_PATH,"washed_plot-",id,".pdf"))
    plot(NULL,
         sub = "hourly time From 2014-03-01 to year 2022-10-31",
         xlab="Date", ylab="NA location",
         xlim = c(0,5881), ylim = c(2013,2023))   
