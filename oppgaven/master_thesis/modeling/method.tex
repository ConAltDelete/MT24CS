...
\subsection{labeling of stations between Nibio and MET}
...
\begin{enumerate}
	\item \begin{lstlisting} "https://lmt.nibio.no/services/rest/weatherstation/getstation?weatherStationId=ID" \end{lstlisting}
	\item \begin{lstlisting} "https://frost.met.no/sources/v0.jsonld?types=SensorSystem&geometry=nearest(POINT(ID.longitude%20ID.latitude))" \end{lstlisting}
	\item \begin{lstlisting} "https://frost.met.no/sources/v0.jsonld?types=SensorSystem&elements=sum(precipitation_amount%20PT1H)&geometry=nearest(POINT(ID.longitude%20ID.latitude))&nearestmaxcount=5" \end{lstlisting}
	\item \begin{lstlisting} "https://frost.met.no/observations/v0.csv?sources=Frost.ID&referencetime=2014-03-1%2F2020-10-31&elements=sum(precipitation_amount%20PT1H)" \end{lstlisting}
\end{enumerate}
Where ID is the Nibio Id for the given station, Frost.ID is the MET id, ID.latitude is the latitude gathered from Nibio, ID.longitude is the longitude gathered from Nibio.
