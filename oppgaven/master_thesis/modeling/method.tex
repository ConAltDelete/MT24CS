...
$$
	\left|\frac{|\Delta T|-µ(|\Delta T|)}{var(|\Delta T|)}\right|> 4C^\circ
$$\todo{Make more clear}
...
%			Epochs & [4,8] \\
%			Units & \(\{2^6,2^7,2^8\}\) \\
%			Lead time & \(12i\) for \(i \in [1,14]\)
%		\end{tabular}
%		\caption[Parameter space for BiLSTM]{The search space for the BiLSTM model. The units define the number of LSTM cells used in the LSTM output. The lead time specifies how many hours the model will take into account when predicting soil temperature. The square brackets indicate an interval including endpoints, while the curly brackets indicate a list of elements.}
%		\label{tab:BiLSTM_params}
%	\end{subtable}
%	\hfill
%	\begin{subtable}[t]{0.45\textwidth}
%		\begin{tabular}{l|c}
...
%			Lag time & [1,14]
%		\end{tabular}
		%\caption[Parameter space for Plauborg]{The search space for the Plauborg model. The square %brackets indicate an interval including endpoints.}
%		\label{tab:plauborg_params}
%	\end{subtable}
%\end{table}
