...
\begin{enumerate}
	\item \begin{verbatim}"https://lmt.nibio.no/services/rest/weatherstation/getstation?weatherStationId=ID"\end{verbatim}
	\item \begin{verbatim}"https://frost.met.no/sources/v0.jsonld?types=SensorSystem&geometry=nearest(POINT(ID.longitude%20ID.latitude))"\end{verbatim}
	\item \begin{verbatim}"https://frost.met.no/sources/v0.jsonld?types=SensorSystem&elements=sum(precipitation_amount%20PT1H)&geometry=nearest(POINT(ID.longitude%20ID.latitude))&nearestmaxcount=5"\end{verbatim}
	\item \begin{verbatim}"https://frost.met.no/observations/v0.csv?sources=Frost.ID&referencetime=2014-03-1%2F2020-10-31&elements=sum(precipitation_amount%20PT1H)"\end{verbatim}
\end{enumerate}
