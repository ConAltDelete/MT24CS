...
\begin{enumerate}
	\item services/rest/weatherstation/getstation?weatherStationId=ID
	\item sources/v0.jsonld?types=SensorSystem\&geometry=nearest(POINT(ID.longitude\%20ID.latitude))
	\item sources/v0.jsonld?types=SensorSystem\&elements=sum(precipitation\_amount\%20PT1H)\&geometry=nearest(POINT(ID.longitude\%20ID.latitude))\&nearestmaxcount=5
	\item https://frost.met.no/observations/v0.csv?sources=Frost.ID\&referencetime=2014-03-1\%2F2020-10-31\&elements=sum(precipitation\_amount\%20PT1H)"
\end{enumerate}
